...
# ✅ BUG FIX: a condição original `result.get("success") and result.get("answer")`
# ocultava a resposta sempre que havia qualquer erro no estado (inclusive "RAG indisponível").
# A resposta final (final_answer) é gerada pelo nó de síntese independentemente de
# erros não-fatais — exibi-la não deve depender do flag success.
    ...
    try:
        ...
        if all_chart_paths:
            for path in all_chart_paths:
                ...
                # ✅ BUG: _generate_time_series_chart gera arquivos como `srag_line_*.html`.
                #    A condição anterior não capturava "line", então o gráfico diário
                #    era classificado como "adicional" e nunca aparecia no checklist.
...
try:
    ...
    print(f"   Taxa de sucesso   : {summary['success_rate']:.1f}%           # ✅ BUG FIX: era :.2% que multiplicava por 100 novamente (ex: 34.28 → 3428%)")
