...
def setup_project_structure(base: str = VOLUME_BASE) -> Dict[str, str]:
    """
    Cria a árvore de diretórios no Databricks Unity Catalog Volume e retorna
    o mapeamento nome -> path para uso nos módulos downstream.

    ✅ v3.1.0 FIX: substituído Path.mkdir() por dbutils.fs.mkdirs().
    Path.mkdir() cria diretórios no sistema de arquivos LOCAL do driver — não
    persiste no Unity Catalog Volume. dbutils.fs.mkdirs() escreve diretamente
    no Volume e funciona em todas as versões do Databricks Runtime.
    ...
    for name, path in paths.items():
        try:
            # ✅ FIX: dbutils.fs.mkdirs() persiste no Unity Catalog Volume
    ...
    # ✅ FIX: verifica existência via dbutils.fs.ls(), não Path.exists()
    # Path.exists() verifica o filesystem local do driver, não o Volume.
    try:
...
try:
    chart_tool = ChartTool(
        ...
        schema       = SCHEMA_GOLD,
        # ✅ FIX: dbutils garante persistência dos HTMLs no Unity Catalog Volume
        # em todas as versões do DBR. Sem dbutils, write_html() usa open() nativo
        # que não persiste em /Volumes/ em DBR < 13.x.
        dbutils      = dbutils,
    )
...
if RAG_ENABLED and RAG_AVAILABLE:
    try:
        ...
        rag_config = RAGConfig(
            ...
            use_citations      = True,
            llm_model          = LLM_MODEL_OPENAI if LLM_PROVIDER == "openai" else LLM_MODEL_DATABRICKS,
        )
        rag_chain = SRAGChain(
            retriever = retriever,
            llm       = llm,
...
for path in [
    "/Volumes/dbx_srag_lab/default/srag_outputs/reports/markdown/",
    "/Volumes/dbx_srag_lab/default/srag_outputs/reports/json/",
    "/Volumes/dbx_srag_lab/default/srag_outputs/charts/custom/",
    "/Volumes/dbx_srag_lab/default/srag_outputs/logs/audit/",
]:
    try:
...
for path in all_charts:
    name = Path(path).name
    # srag_line_* e variantes de "diario" → gráfico diário obrigatório.
    # "multi_line" é excluído para não confundir com o breakdown viral.
    if (
        "time_series" in name
        or "diario"   in name
        or "daily"    in name
        or ("line" in name and "multi_line" not in name)
    ):
        daily_charts.append(path)
    # ✅ FIX: removido "bar" da condição — gráfico geográfico (srag_bar_*) era
    # capturado erroneamente como mensal. Apenas prefixo "mensal" / "monthly"
    # identifica o gráfico obrigatório gerado por _generate_monthly_chart().
...
for p in all_charts:
    tag = ""
    n   = Path(p).name
    if p in daily_charts:
        tag = " [diario]"
    elif p in monthly_charts:
        tag = " [mensal]"
    print(f"  {n}{tag}")
...
try:
    ...
    # ✅ FIX: dbutils.fs.put() persiste no Unity Catalog Volume em todas as versões
    # do DBR. Path.write_text() usa open() nativo e NÃO persiste em /Volumes/ em
    # DBR < 13.x — os arquivos eram criados no filesystem local do driver e perdidos.
    ...
    # ✅ FIX: idem — dbutils.fs.put() no lugar de Path.write_text()
    dbutils.fs.put(
        json_path,
...
try:
    ...
    # ✅ FIX: export_to_json usa open() internamente — não persiste em /Volumes/.
    # Estratégia: obtém o JSON como string e persiste via dbutils.fs.put().
    try:
        import json as _json
        audit_json_str = _json.dumps(
            [e if isinstance(e, dict) else vars(e) for e in audit_logger.logs],
            indent=2, ensure_ascii=False, default=str,
        )
        dbutils.fs.put(audit_json_path, audit_json_str, overwrite=True)
    except Exception as _ej:
        # Fallback: tenta export_to_json nativo (pode não persistir em DBR < 13.x)
        print(f"  aviso: dbutils.fs.put falhou ({_ej}) — tentando export_to_json nativo")
